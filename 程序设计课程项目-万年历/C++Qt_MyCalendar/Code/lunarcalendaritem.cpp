...
{
    hover = false; //初始状态不悬浮，不点击，不选中
    pressed = false;

    select = false;
    showLunar = true; //显示农历
    bgImage = ":/image/bg_calendar.png";
    selectType = SelectType_Rect; //初始为矩形

    date = QDate::currentDate(); //返回当前日期
    lunar = "初一";//初始为初一
    dayType = DayType_MonthCurrent; //当月天数

    borderColor = QColor(180, 180, 180); //边框颜色，灰色
    weekColor = QColor(255, 0, 0); //周末颜色，红色
    superColor = QColor(255, 129, 6); //角标颜色，金色
    lunarColor = QColor(55, 156, 238); //农历颜色，蓝色

    currentTextColor = QColor(0, 0, 0); //当前月文字颜色，黑色
    otherTextColor = QColor(200, 200, 200); //其他月文字颜色，灰色
    selectTextColor = QColor(255, 255, 255); //选中日期文字颜色，白色
    hoverTextColor = QColor(250, 250, 250); //悬浮日期文字颜色，白色

    currentLunarColor = QColor(150, 150, 150); //当前农历月文字颜色，深灰色
    otherLunarColor = QColor(200, 200, 200); //其他农历月文字颜色，灰色
    selectLunarColor = QColor(255, 255, 255); //选中日期农历文字颜色，白色
    hoverLunarColor = QColor(250, 250, 250); //悬浮日期农历文字颜色，白色

    currentBgColor = QColor(255, 255, 255); //当前月背景颜色，白色
    otherBgColor = QColor(240, 240, 240); //其他月背景颜色，浅灰色
    selectBgColor = QColor(208, 47, 18); //选中日期背景颜色，红色
    hoverBgColor = QColor(204, 183, 180); //悬浮日期颜色，灰色
}
...
void LunarCalendarItem::drawBg(QPainter *painter)
{
    ...
    //根据当前类型选择对应的颜色
    QColor bgColor = currentBgColor; //当前月
    if (dayType == DayType_MonthPre || dayType == DayType_MonthNext) {
        bgColor = otherBgColor; //其他月
    }
